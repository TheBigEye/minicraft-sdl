		...
		if (item->add.tool.type == SHOVEL) {
			if (player_payStamina(player, 4 - item->add.tool.level)) {
				...
				if (random_next_int(random, 5) == 0) {
					...
					itementity_create(item_entity, item, (xt * 16) + random_next_int(random, 10) + 3, (yt * 16) + random_next_int(random, 10) + 3);
...
void grasstile_render(TileID id, Screen* screen, Level* level, int x, int y) {
	...
	if (!u && !l) {
        ...
        render_screen(screen, (x * 16) + 0, (y * 16) + 0, (l ? 11 : 12) + (u ? 0 : 1) * 32, transitionColor, 0);
    }

	if (!u && !r) {
        render_screen(screen, (x * 16) + 8, (y * 16) + 0, 1, col, 0);
    } else {
        render_screen(screen, (x * 16) + 8, (y * 16) + 0, (r ? 13 : 12) + (u ? 0 : 1) * 32, transitionColor, 0);
    }

	if (!d && !l) {
        render_screen(screen, (x * 16) + 0, (y * 16) + 8, 2, col, 0);
    } else {
        render_screen(screen, (x * 16) + 0, (y * 16) + 8, (l ? 11 : 12) + (d ? 2 : 1) * 32, transitionColor, 0);
    }

	if (!d && !r) {
        render_screen(screen, (x * 16) + 8, (y * 16) + 8, 3, col, 0);
    } else {
        render_screen(screen, (x * 16) + 8, (y * 16) + 8, (r ? 13 : 12) + (d ? 2 : 1) * 32, transitionColor, 0);
    }
}
...
void grasstile_tick(TileID id, Level* level, int xt, int yt){
	...
	if (level_get_tile(level, xn, yn) == DIRT) {
